// SVG Icons based on YouTube's design

import { cn } from "../../utils";

export const MenuIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M20 5H4a1 1 0 000 2h16a1 1 0 100-2Zm0 6H4a1 1 0 000 2h16a1 1 0 000-2Zm0 6H4a1 1 0 000 2h16a1 1 0 000-2Z" />
  </svg>
);

export const JeffTubeLogo = () => (
  <div className="flex items-center gap-1">

    <img src="/logo.png" alt="JeffTube" className="h-5" />
    <span className="text-xl font-semibold tracking-tight">JeffTube</span>
  </div>
);

export const SearchIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M20.87 20.17l-5.59-5.59C16.35 13.35 17 11.75 17 10c0-3.87-3.13-7-7-7s-7 3.13-7 7 3.13 7 7 7c1.75 0 3.35-.65 4.58-1.71l5.59 5.59.7-.71zM10 16c-3.31 0-6-2.69-6-6s2.69-6 6-6 6 2.69 6 6-2.69 6-6 6z" />
  </svg>
);

export const MicIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 3c-1.66 0-3 1.37-3 3.07v5.86c0 1.7 1.34 3.07 3 3.07s3-1.37 3-3.07V6.07C15 4.37 13.66 3 12 3zm6.5 9h-1c0 3.03-2.47 5.5-5.5 5.5S6.5 15.03 6.5 12h-1c0 3.24 2.39 5.93 5.5 6.41V21h2v-2.59c3.11-.48 5.5-3.17 5.5-6.41z" />
  </svg>
);

export const CreateIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M14 13h-3v3H9v-3H6v-2h3V8h2v3h3v2zm3-7H3v12h14v-6.39l4 1.83V8.56l-4 1.83V6m1-1v3.83L22 7v8l-4-1.83V19H2V5h16z" />
  </svg>
);

export const NotificationIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M10 20h4c0 1.1-.9 2-2 2s-2-.9-2-2zm10-2.65V19H4v-1.65l2-1.88v-5.15C6 7.4 7.56 5.1 10 4.34v-.38c0-1.42 1.49-2.5 2.99-1.76.65.32 1.01 1.03 1.01 1.76v.39c2.44.75 4 3.06 4 5.98v5.15l2 1.87zm-1 .42-2-1.88v-5.47c0-2.47-1.19-4.36-3.13-5.1-1.26-.48-2.49-.48-3.75 0C8.18 6.06 7 7.96 7 10.42v5.47l-2 1.88V18h14v-.23z" />
  </svg>
);

export const HomeIcon = ({ active = false }: { active?: boolean }) => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    {active ? (
      <path d="M4 21V10.08l8-6.96 8 6.96V21h-6v-6h-4v6H4z" />
    ) : (
      <path d="m12 4.44 7 6.09V20h-4v-6H9v6H5v-9.47l7-6.09m0-1.32-8 6.96V21h6v-6h4v6h6V10.08l-8-6.96z" />
    )}
  </svg>
);

export const ShortsIcon = ({ active = false }: { active?: boolean }) => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    {active ? (
      <path d="m13.974 2.052-8 4.7a4 4 0 00.385 7.097l.942.423-1.327.78a4 4 0 004.052 6.897l8-4.7a4.001 4.001 0 00-.384-7.096L16.7 9.73l1.326-.78a4 4 0 10-4.052-6.897ZM10 15V9l5 3-5 3Z" />
    ) : (
      <path d="M10 14.65v-5.3L15 12l-5 2.65zm7.77-4.33-1.2-.5L18 9.06c1.84-.96 2.53-3.23 1.56-5.06s-3.24-2.53-5.07-1.56L6 6.94c-1.29.68-2.07 2.04-2 3.49.07 1.42.93 2.67 2.22 3.25.03.01 1.2.5 1.2.5L6 14.93c-1.83.97-2.53 3.24-1.56 5.07.97 1.83 3.24 2.53 5.07 1.56l8.5-4.5c1.29-.68 2.06-2.04 1.99-3.49-.07-1.42-.94-2.68-2.23-3.25zm-.23 5.86-8.5 4.5c-1.34.71-3.01.2-3.72-1.14-.71-1.34-.2-3.01 1.14-3.72l2.04-1.08v-1.21l-.69-.28-1.11-.46c-.99-.41-1.65-1.35-1.7-2.41-.05-1.06.52-2.06 1.46-2.56l8.5-4.5c1.34-.71 3.01-.2 3.72 1.14.71 1.34.2 3.01-1.14 3.72L15.5 9.26v1.21l1.8.74c.99.41 1.65 1.35 1.7 2.41.05 1.06-.52 2.06-1.46 2.56z" />
    )}
  </svg>
);

export const SubscriptionsIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M10 18v-6l5 3-5 3zm7-15H7v1h10V3zm3 3H4v1h16V6zm2 3H2v12h20V9zM3 20V10h18v10H3z" />
  </svg>
);

export const HistoryIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M8.76 1.487a11 11 0 11-7.54 12.706 1 1 0 011.96-.4 9 9 0 0014.254 5.38A9 9 0 0016.79 4.38 9 9 0 004.518 7H7a1 1 0 010 2H1V3a1 1 0 012 0v2.678a11 11 0 015.76-4.192ZM12 6a1 1 0 00-1 1v5.58l.504.288 3.5 2a1 1 0 10.992-1.736L13 11.42V7a1 1 0 00-1-1Z" />
  </svg>
);

export const PlaylistIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M16 15.395a.5.5 0 01.762-.426L22.5 18.5l-5.738 3.531a.5.5 0 01-.762-.425v-6.212ZM14 19H4a1 1 0 110-2h10v2Zm6-8a1 1 0 110 2H4a1 1 0 110-2h16Zm0-6a1 1 0 110 2H4a1 1 0 010-2h16Z" />
  </svg>
);

export const WatchLaterIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 1C5.925 1 1 5.925 1 12s4.925 11 11 11 11-4.925 11-11S18.075 1 12 1Zm0 2a9 9 0 110 18.001A9 9 0 0112 3Zm0 3a1 1 0 00-1 1v5.565l.485.292 3.33 2a1 1 0 001.03-1.714L13 11.435V7a1 1 0 00-1-1Z" />
  </svg>
);

export const LikedVideosIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M9.221 1.795a1 1 0 011.109-.656l1.04.173a4 4 0 013.252 4.784L14 9h4.061a3.664 3.664 0 013.576 2.868A3.68 3.68 0 0121 14.85l.02.087A3.815 3.815 0 0120 18.5v.043l-.01.227a2.82 2.82 0 01-.135.663l-.106.282A3.754 3.754 0 0116.295 22h-3.606l-.392-.007a12.002 12.002 0 01-5.223-1.388l-.343-.189-.27-.154a2.005 2.005 0 00-.863-.26l-.13-.004H3.5a1.5 1.5 0 01-1.5-1.5V12.5A1.5 1.5 0 013.5 11h1.79l.157-.013a1 1 0 00.724-.512l.063-.145 2.987-8.535Zm-1.1 9.196A3 3 0 015.29 13H4v4.998h1.468a4 4 0 011.986.528l.27.155.285.157A10 10 0 0012.69 20h3.606c.754 0 1.424-.483 1.663-1.2l.03-.126a.819.819 0 00.012-.131v-.872l.587-.586c.388-.388.577-.927.523-1.465l-.038-.23-.02-.087-.21-.9.55-.744A1.663 1.663 0 0018.061 11H14a2.002 2.002 0 01-1.956-2.418l.623-2.904a2 2 0 00-1.626-2.392l-.21-.035-2.71 7.741Z" />
  </svg>
);

export const YourVideosIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M21 3H3a2 2 0 00-2 2v14a2 2 0 002 2h18a2 2 0 002-2V5a2 2 0 00-2-2ZM3 19V5h18v14H3Zm13-7L9.5 8v8l6.5-4Z" />
  </svg>
);

export const DownloadIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 2a1 1 0 00-1 1v11.586l-4.293-4.293a1 1 0 10-1.414 1.414L12 18.414l6.707-6.707a1 1 0 10-1.414-1.414L13 14.586V3a1 1 0 00-1-1Zm7 18H5a1 1 0 000 2h14a1 1 0 000-2Z" />
  </svg>
);

export const MusicIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 4v9.38c-.73-.84-1.8-1.38-3-1.38-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V8h6V4h-7zM9 19c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm9-12h-5V5h5v2z" />
  </svg>
);

export const MoviesIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M4 6l2 4h14v10H4V6zm0-1c-.55 0-1 .45-1 1v14c0 .55.45 1 1 1h16c.55 0 1-.45 1-1V9c0-.55-.45-1-1-1h-3V4.62c0-.34-.28-.62-.62-.62-.19 0-.36.08-.49.22L13.41 7H8.59L6.11 4.22C5.98 4.08 5.81 4 5.62 4c-.34 0-.62.28-.62.62V8H4zm7 3H6.41l2-3H10l1 3zm5 0h-4l-1-3h4l1 3zm2-3v3h-1l-1-3h2z" />
  </svg>
);

export const LiveIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M14 12c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2zM8.48 8.45l-.71-.7C6.68 8.83 6 10.34 6 12s.68 3.17 1.77 4.25l.71-.71C7.57 14.64 7 13.39 7 12s.57-2.64 1.48-3.55zm7.75-.7-.71.71c.91.9 1.48 2.15 1.48 3.54s-.57 2.64-1.48 3.55l.71.71C17.32 15.17 18 13.66 18 12s-.68-3.17-1.77-4.25zM5.65 5.63l-.7-.71C3.13 6.73 2 9.24 2 12s1.13 5.27 2.95 7.08l.71-.71C4.02 16.74 3 14.49 3 12s1.02-4.74 2.65-6.37zm13.4-.71-.71.71C19.98 7.26 21 9.51 21 12s-1.02 4.74-2.65 6.37l.71.71C20.87 17.27 22 14.76 22 12s-1.13-5.27-2.95-7.08z" />
  </svg>
);

export const ChevronDownIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 15.7l-5.6-5.6.7-.7 4.9 4.9 4.9-4.9.7.7z" />
  </svg>
);

export const ChevronRightIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M9.4 18.4l-.7-.7 5.6-5.6-5.7-5.7.7-.7 6.4 6.4-6.3 6.3z" />
  </svg>
);

export const MoreVertIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 16.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5-1.5-.67-1.5-1.5.67-1.5 1.5-1.5zM10.5 12c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5-1.5.67-1.5 1.5zm0-6c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5-1.5.67-1.5 1.5z" />
  </svg>
);

export const BellIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
    <path d="M10 20h4c0 1.1-.9 2-2 2s-2-.9-2-2zm10-2.65V19H4v-1.65l2-1.88v-5.15c0-2.92 1.56-5.22 4-5.98V3.96c0-1.42 1.49-2.5 2.99-1.76.65.32 1.01 1.03 1.01 1.76v.39c2.44.75 4 3.06 4 5.98v5.15l2 1.87z" />
  </svg>
);

export const VerifiedIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor">
    <path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zM9.8 17.3l-4.2-4.1L7 11.8l2.8 2.7L17 7.4l1.4 1.4-8.6 8.5z" />
  </svg>
);

export const PlayIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M8 5v14l11-7z" />
  </svg>
);

export const JeffTubePremiumIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path
      d="M10 9.35L15 12l-5 2.65zM12 6a54.36 54.36 0 00-7.56.38A1.53 1.53 0 003.38 7.4 24.47 24.47 0 003 12a24.47 24.47 0 00.38 4.6 1.53 1.53 0 001.06 1 54.36 54.36 0 007.56.4 54.36 54.36 0 007.56-.38 1.53 1.53 0 001.06-1A24.47 24.47 0 0021 12a24.47 24.47 0 00-.38-4.6 1.53 1.53 0 00-1.06-1A54.36 54.36 0 0012 6zm0-1a56.19 56.19 0 017.81.4 2.53 2.53 0 011.75 1.7A25.87 25.87 0 0122 12a25.87 25.87 0 01-.44 4.9 2.53 2.53 0 01-1.75 1.7A56.19 56.19 0 0112 19a56.19 56.19 0 01-7.81-.4 2.53 2.53 0 01-1.75-1.7A25.87 25.87 0 012 12a25.87 25.87 0 01.44-4.9 2.53 2.53 0 011.75-1.7A56.19 56.19 0 0112 5z"
      fill="#FF0000"
    />
  </svg>
);

export const JeffTubeStudioIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path
      d="M10 9.35L15 12l-5 2.65zM12 6a54.36 54.36 0 00-7.56.38A1.53 1.53 0 003.38 7.4 24.47 24.47 0 003 12a24.47 24.47 0 00.38 4.6 1.53 1.53 0 001.06 1 54.36 54.36 0 007.56.4 54.36 54.36 0 007.56-.38 1.53 1.53 0 001.06-1A24.47 24.47 0 0021 12a24.47 24.47 0 00-.38-4.6 1.53 1.53 0 00-1.06-1A54.36 54.36 0 0012 6zm0-1a56.19 56.19 0 017.81.4 2.53 2.53 0 011.75 1.7A25.87 25.87 0 0122 12a25.87 25.87 0 01-.44 4.9 2.53 2.53 0 01-1.75 1.7A56.19 56.19 0 0112 19a56.19 56.19 0 01-7.81-.4 2.53 2.53 0 01-1.75-1.7A25.87 25.87 0 012 12a25.87 25.87 0 01.44-4.9 2.53 2.53 0 011.75-1.7A56.19 56.19 0 0112 5z"
      fill="#FF0000"
    />
  </svg>
);

export const KeyboardIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor">
    <path d="M20 5H4c-1.1 0-1.99.9-1.99 2L2 17c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm-9 3h2v2h-2V8zm0 3h2v2h-2v-2zM8 8h2v2H8V8zm0 3h2v2H8v-2zm-1 2H5v-2h2v2zm0-3H5V8h2v2zm9 7H8v-2h8v2zm0-4h-2v-2h2v2zm0-3h-2V8h2v2zm3 3h-2v-2h2v2zm0-3h-2V8h2v2z" />
  </svg>
);

export const SunIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
    <path d="M12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm18 0h2c.55 0 1-.45 1-1s-.45-1-1-1h-2c-.55 0-1 .45-1 1s.45 1 1 1zM11 2v2c0 .55.45 1 1 1s1-.45 1-1V2c0-.55-.45-1-1-1s-1 .45-1 1zm0 18v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zM5.99 4.58a.996.996 0 00-1.41 0 .996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37a.996.996 0 00-1.41 0 .996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0a.996.996 0 000-1.41l-1.06-1.06zm1.06-10.96a.996.996 0 000-1.41.996.996 0 00-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM7.05 18.36a.996.996 0 000-1.41.996.996 0 00-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06z" />
  </svg>
);

export const MoonIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
    <path d="M12 3a9 9 0 109 9c0-.46-.04-.92-.1-1.36a5.389 5.389 0 01-4.4 2.26 5.403 5.403 0 01-3.14-9.8c-.44-.06-.9-.1-1.36-.1z" />
  </svg>
);

export const SystemIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
    <path d="M20 3H4c-1.1 0-2 .9-2 2v11c0 1.1.9 2 2 2h3l-1 1v2h12v-2l-1-1h3c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 13H4V5h16v11z" />
  </svg>
);

export const QuestionIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17h-2v-2h2v2zm2.07-7.75l-.9.92C13.45 12.9 13 13.5 13 15h-2v-.5c0-1.1.45-2.1 1.17-2.83l1.24-1.26c.37-.36.59-.86.59-1.41 0-1.1-.9-2-2-2s-2 .9-2 2H8c0-2.21 1.79-4 4-4s4 1.79 4 4c0 .88-.36 1.68-.93 2.25z" />
  </svg>
);

export const CloseIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" />
  </svg>
);

export const LikeIcon = ({ className, filled = false }: { className?: string, filled?: boolean }) => (
  <svg viewBox="0 0 24 24" className={cn("w-6 h-6", className)} fill="currentColor">
    <g transform={filled ? "translate(12, 12)" : "translate(12, 12)"}>
      {filled ? (
        // Filled version - outer path, scaled down
        <path d="M-2.778,-10.205C-2.618,-10.664 -2.15,-10.941 -1.67,-10.862L-0.63,-10.688C1.62,-10.312 3.1,-8.135 2.622,-5.905L2,-3H6.061C7.778,-3 9.265,-1.808 9.638,-0.132C9.868,0.904 9.632,1.996 9,2.849L9.02,2.937C9.319,4.221 8.933,5.568 8,6.5V6.543L7.991,6.769C7.973,6.995 7.927,7.218 7.855,7.433L7.749,7.715C7.164,9.091 5.809,10 4.295,10H0.689L0.298,9.993C-1.526,9.933 -3.313,9.459 -4.926,8.605L-5.269,8.416L-5.538,8.262C-5.803,8.11 -6.099,8.021 -6.401,8.002L-6.531,7.998H-8.5C-9.328,7.998 -10,7.326 -10,6.498V0.5C-10,-0.328 -9.328,-1 -8.5,-1H-6.71L-6.553,-1.013C-6.245,-1.062 -5.977,-1.251 -5.829,-1.526L-5.766,-1.67L-2.778,-10.205Z"
        />
      ) : (
        // Outline version
        <path d="M-2.778,-10.205C-2.618,-10.664 -2.15,-10.941 -1.67,-10.862C-1.67,-10.862 -0.63,-10.688 -0.63,-10.688C1.62,-10.312 3.1,-8.135 2.622,-5.905C2.622,-5.905 2,-3 2,-3C2,-3 6.061,-3 6.061,-3C7.778,-3 9.265,-1.808 9.638,-0.132C9.868,0.904 9.632,1.996 9,2.849C9,2.849 9.02,2.937 9.02,2.937C9.319,4.221 8.933,5.568 8,6.5C8,6.5 8,6.543 8,6.543C8,6.543 7.991,6.769 7.991,6.769C7.973,6.995 7.927,7.218 7.855,7.433C7.855,7.433 7.749,7.715 7.749,7.715C7.164,9.091 5.809,10 4.295,10C4.295,10 0.689,10 0.689,10C0.689,10 0.298,9.993 0.298,9.993C-1.526,9.933 -3.313,9.459 -4.926,8.605C-4.926,8.605 -5.269,8.416 -5.269,8.416C-5.269,8.416 -5.538,8.262 -5.538,8.262C-5.803,8.11 -6.099,8.021 -6.401,8.002C-6.401,8.002 -6.531,7.998 -6.531,7.998C-6.531,7.998 -8.5,7.998 -8.5,7.998C-9.328,7.998 -10,7.326 -10,6.498C-10,6.498 -10,0.5 -10,0.5C-10,-0.328 -9.328,-1 -8.5,-1C-8.5,-1 -6.71,-1 -6.71,-1C-6.71,-1 -6.553,-1.013 -6.553,-1.013C-6.245,-1.062 -5.977,-1.251 -5.829,-1.526C-5.829,-1.526 -5.766,-1.67 -5.766,-1.67C-5.766,-1.67 -2.778,-10.205 -2.778,-10.205z M-3.878,-1.009C-4.299,0.194 -5.435,1 -6.71,1C-6.71,1 -8,1 -8,1C-8,1 -8,5.998 -8,5.998C-8,5.998 -6.531,5.998 -6.531,5.998C-5.835,5.998 -5.151,6.18 -4.546,6.526C-4.546,6.526 -4.275,6.68 -4.275,6.68C-4.275,6.68 -3.99,6.838 -3.99,6.838C-2.55,7.6 -0.943,8 0.689,8C0.689,8 4.295,8 4.295,8C5.049,8 5.719,7.516 5.958,6.801C5.958,6.801 5.989,6.674 5.989,6.674C5.996,6.63 6,6.587 6,6.543C6,6.543 6,5.671 6,5.671C6,5.671 6.587,5.085 6.587,5.085C6.975,4.697 7.164,4.158 7.11,3.62C7.11,3.62 7.072,3.389 7.072,3.389C7.072,3.389 7.052,3.303 7.052,3.303C7.052,3.303 6.843,2.402 6.843,2.402C6.843,2.402 7.392,1.659 7.392,1.659C7.682,1.268 7.788,0.767 7.685,0.302C7.515,-0.459 6.841,-1 6.061,-1C6.061,-1 2,-1 2,-1C1.396,-1 0.824,-1.274 0.444,-1.743C0.065,-2.213 -0.082,-2.829 0.044,-3.419C0.044,-3.419 0.667,-6.323 0.667,-6.323C0.906,-7.438 0.166,-8.527 -0.959,-8.715C-0.959,-8.715 -1.169,-8.75 -1.169,-8.75C-1.169,-8.75 -3.878,-1.009 -3.878,-1.009z" />
      )}
    </g>
  </svg>
);


export const DislikeIcon = ({ className, filled = false }: { className?: string, filled?: boolean }) => (
  <svg viewBox="0 0 24 24" className={cn("w-6 h-6 rotate-180", className)} fill="currentColor">
    <g transform="translate(12, 12)">
      {filled ? (
        <path d="M-2.778,-10.205C-2.618,-10.664 -2.15,-10.941 -1.67,-10.862L-0.63,-10.688C1.62,-10.312 3.1,-8.135 2.622,-5.905L2,-3H6.061C7.778,-3 9.265,-1.808 9.638,-0.132C9.868,0.904 9.632,1.996 9,2.849L9.02,2.937C9.319,4.221 8.933,5.568 8,6.5V6.543L7.991,6.769C7.973,6.995 7.927,7.218 7.855,7.433L7.749,7.715C7.164,9.091 5.809,10 4.295,10H0.689L0.298,9.993C-1.526,9.933 -3.313,9.459 -4.926,8.605L-5.269,8.416L-5.538,8.262C-5.803,8.11 -6.099,8.021 -6.401,8.002L-6.531,7.998H-8.5C-9.328,7.998 -10,7.326 -10,6.498V0.5C-10,-0.328 -9.328,-1 -8.5,-1H-6.71L-6.553,-1.013C-6.245,-1.062 -5.977,-1.251 -5.829,-1.526L-5.766,-1.67L-2.778,-10.205Z"
        />
      ) : (
        <path d="M-2.778,-10.205C-2.618,-10.664 -2.15,-10.941 -1.67,-10.862C-1.67,-10.862 -0.63,-10.688 -0.63,-10.688C1.62,-10.312 3.1,-8.135 2.622,-5.905C2.622,-5.905 2,-3 2,-3C2,-3 6.061,-3 6.061,-3C7.778,-3 9.265,-1.808 9.638,-0.132C9.868,0.904 9.632,1.996 9,2.849C9,2.849 9.02,2.937 9.02,2.937C9.319,4.221 8.933,5.568 8,6.5C8,6.5 8,6.543 8,6.543C8,6.543 7.991,6.769 7.991,6.769C7.973,6.995 7.927,7.218 7.855,7.433C7.855,7.433 7.749,7.715 7.749,7.715C7.164,9.091 5.809,10 4.295,10C4.295,10 0.689,10 0.689,10C0.689,10 0.298,9.993 0.298,9.993C-1.526,9.933 -3.313,9.459 -4.926,8.605C-4.926,8.605 -5.269,8.416 -5.269,8.416C-5.269,8.416 -5.538,8.262 -5.538,8.262C-5.803,8.11 -6.099,8.021 -6.401,8.002C-6.401,8.002 -6.531,7.998 -6.531,7.998C-6.531,7.998 -8.5,7.998 -8.5,7.998C-9.328,7.998 -10,7.326 -10,6.498C-10,6.498 -10,0.5 -10,0.5C-10,-0.328 -9.328,-1 -8.5,-1C-8.5,-1 -6.71,-1 -6.71,-1C-6.71,-1 -6.553,-1.013 -6.553,-1.013C-6.245,-1.062 -5.977,-1.251 -5.829,-1.526C-5.829,-1.526 -5.766,-1.67 -5.766,-1.67C-5.766,-1.67 -2.778,-10.205 -2.778,-10.205z M-3.878,-1.009C-4.299,0.194 -5.435,1 -6.71,1C-6.71,1 -8,1 -8,1C-8,1 -8,5.998 -8,5.998C-8,5.998 -6.531,5.998 -6.531,5.998C-5.835,5.998 -5.151,6.18 -4.546,6.526C-4.546,6.526 -4.275,6.68 -4.275,6.68C-4.275,6.68 -3.99,6.838 -3.99,6.838C-2.55,7.6 -0.943,8 0.689,8C0.689,8 4.295,8 4.295,8C5.049,8 5.719,7.516 5.958,6.801C5.958,6.801 5.989,6.674 5.989,6.674C5.996,6.63 6,6.587 6,6.543C6,6.543 6,5.671 6,5.671C6,5.671 6.587,5.085 6.587,5.085C6.975,4.697 7.164,4.158 7.11,3.62C7.11,3.62 7.072,3.389 7.072,3.389C7.072,3.389 7.052,3.303 7.052,3.303C7.052,3.303 6.843,2.402 6.843,2.402C6.843,2.402 7.392,1.659 7.392,1.659C7.682,1.268 7.788,0.767 7.685,0.302C7.515,-0.459 6.841,-1 6.061,-1C6.061,-1 2,-1 2,-1C1.396,-1 0.824,-1.274 0.444,-1.743C0.065,-2.213 -0.082,-2.829 0.044,-3.419C0.044,-3.419 0.667,-6.323 0.667,-6.323C0.906,-7.438 0.166,-8.527 -0.959,-8.715C-0.959,-8.715 -1.169,-8.75 -1.169,-8.75C-1.169,-8.75 -3.878,-1.009 -3.878,-1.009z" />
      )}
    </g>
  </svg>
);

export const ShareIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M10 3.158V7.51c-5.428.223-8.27 3.75-8.875 11.199-.04.487-.07.975-.09 1.464l-.014.395c-.014.473.578.684.88.32.302-.368.61-.73.925-1.086l.244-.273c1.79-1.967 3-2.677 4.93-2.917a18.011 18.011 0 012-.112v4.346a1 1 0 001.646.763l9.805-8.297 1.55-1.31-1.55-1.31-9.805-8.297A1 1 0 0010 3.158Zm2 6.27v.002-4.116l7.904 6.688L12 18.689v-4.212l-2.023.024c-1.935.022-3.587.17-5.197 1.024a9 9 0 00-1.348.893c.355-1.947.916-3.39 1.63-4.425 1.062-1.541 2.607-2.385 5.02-2.485L12 9.428Z" />
  </svg>
);

export const DownloadVideoIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 2a1 1 0 00-1 1v11.586l-4.293-4.293a1 1 0 10-1.414 1.414L12 18.414l6.707-6.707a1 1 0 10-1.414-1.414L13 14.586V3a1 1 0 00-1-1Zm7 18H5a1 1 0 000 2h14a1 1 0 000-2Z" />
  </svg>
);

export const MoreHorizIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M6 10a2 2 0 100 4 2 2 0 000-4Zm6 0a2 2 0 100 4 2 2 0 000-4Zm6 0a2 2 0 100 4 2 2 0 000-4Z" />
  </svg>
);

export const ChevronLeftIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M14.6 18.4l-6.4-6.4 6.4-6.4.7.7-5.6 5.7 5.7 5.7z" />
  </svg>
);

export const PauseIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" />
  </svg>
);

export const VolumeIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z" />
  </svg>
);

export const VolumeMutedIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" />
  </svg>
);

export const CommentIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M1 6a4 4 0 014-4h14a4 4 0 014 4v10a4 4 0 01-4 4h-4.8l-5.105 2.836A1.41 1.41 0 017 21.604V20H5a4 4 0 01-4-4V6Zm8 12v2.601l4.229-2.35.453-.251H19a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2h4ZM7 8a1 1 0 000 2h10a1 1 0 100-2H7Zm-1 5a1 1 0 001 1h6a1 1 0 000-2H7a1 1 0 00-1 1Z" />
  </svg>
);

export const NavUpIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M19.707 10.293 12 2.586l-7.707 7.707a1 1 0 101.414 1.414L11 6.414V20a1 1 0 002 0V6.414l5.293 5.293a1 1 0 101.414-1.414Z" />
  </svg>
);

export const NavDownIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 3a1 1 0 00-1 1v13.586l-5.293-5.293a1 1 0 10-1.414 1.414L12 21.414l7.707-7.707a1 1 0 10-1.414-1.414L13 17.586V4a1 1 0 00-1-1Z" />
  </svg>
);

export const SettingsIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M12 9.5a2.5 2.5 0 100 5 2.5 2.5 0 000-5zM10 12a2 2 0 114 0 2 2 0 01-4 0z" />
    <path d="M19.14 12.94c.04-.31.06-.63.06-.94 0-.31-.02-.63-.06-.94l2.03-1.58a.49.49 0 00.12-.61l-1.92-3.32a.488.488 0 00-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94l-.36-2.54a.484.484 0 00-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96c-.22-.08-.47 0-.59.22L2.74 8.87c-.12.21-.08.47.12.61l2.03 1.58c-.04.31-.06.63-.06.94s.02.63.06.94l-2.03 1.58a.49.49 0 00-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.38 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.56 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32c.12-.22.07-.47-.12-.61l-2.01-1.58zM12 15.5a3.5 3.5 0 110-7 3.5 3.5 0 010 7z" />
  </svg>
);

export const SignOutIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M20 3v18H8v-1h11V4H8V3h12zm-8.9 12.1.7.7 4.4-4.4L11.8 7l-.7.7 3.1 3.1H3v1h11.3l-3.2 3.3z" />
  </svg>
);

export const SortIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
    <path d="M21 5H3a1 1 0 000 2h18a1 1 0 100-2Zm-6 6H3a1 1 0 000 2h12a1 1 0 000-2Zm-6 6H3a1 1 0 000 2h6a1 1 0 000-2Z">
    </path>
  </svg>
);
